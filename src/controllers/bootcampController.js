...
exports.getImageCover = asyncHandler(async (req, res, next) => {
  ...
  if (!bootcamp || !bootcamp.imageCover) {
    return next(
      new AppError('No bootcamp or coverImage found with that ID', 404)
    )
  }
...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (publishedBootcamp && req.user.role !== 'admin') {
    return next(new AppError('You have already published a bootcamp', 400))
  }
...
exports.getAllBootcamps = asyncHandler(async (req, res, next) => {
  const features = new Features(Bootcamp.find(), req.query)
    .filter()
    .sort()
    .limitFields()
    .paginate()
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      new AppError('You are not authorized to update this bootcamp', 401)
    )
  }
...
exports.deleteBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      new AppError('You are not authorized to delete this bootcamp', 401)
    )
  }
